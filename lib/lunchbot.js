...
Lunchbot.prototype._onTextMessage = function (message) {
    ...
    } else if (message.text.toLowerCase().startsWith('register ')) {
        this._onRegister(message);
    } else if (message.text.toLowerCase().startsWith('order ')) {
...
Lunchbot.prototype._onHelp = function (message) {
    ...
    info.push("register <Ваше имя> - добавляет имя пользователя в гугл док для дальнейшего использования. Пример:\nregister Олег Майами");
...
Lunchbot.prototype._onRegister = function (message) {
    var docUserName = message.text.substring('mynameis '.length);
    var that = this;
    this.lunchdoc.findUser(docUserName, function(result) {
        if (result) {
            var errorMessage = "<@{}>, вы уже зарегистрированы в документе как {}".format(that.users[message.user].name, docUserName);
            that._postMessage(message.channel, errorMessage);
        } else {
            that.lunchdoc.createUser(docUserName, function(err) {
                MongoClient.connect(that.mongodbUrl, function(err, db) {
                    var users = db.collection("users");
                    users.updateOne({slackid:message.user}, {slackid:message.user, doc_username: docUserName}, {upsert:true}, function(err, db) {
                        console.log("Mapped " + message.user + " with " + docUserName);
                    });
                    db.close();
                    var successMessage = "Спасибо, <@{}>, Ваше имя было сохранено как {}".format(that.users[message.user].name, docUserName);
        ...
        users.findOne({slackid:message.user}, function(err, document) {
            if (document) {
                ...
                var noUserFoundMsg =
                    ...
                    "Пожалуйста, зарегестрируйтесь с помощью команды register <Ваше имя>. " +
                    "Пример:\nregister Олег Майами";
