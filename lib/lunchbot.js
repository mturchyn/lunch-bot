...
Lunchbot.prototype._onMenu = function (message) {
    ...
    async.series([
        function setAuth(step) {
            var creds = require('../conf/LunchSmartBot-6c7ea4acbbc2.json');
...
Lunchbot.prototype._postMessage = function (slackMessage, text, params) {
    if (this.isChannelMessage(slackMessage)) {
        this.postMessageToGroup(this.memberChannelsAndGroups[slackMessage.channel], text, params || {});
    } else if (this.isGroupMessage(slackMessage)) {
        this.postMessageToChannel(this.memberChannelsAndGroups[slackMessage.channel], text, params || {});
    } else {
        console.error("Slack Message is not from Group or Channel:\n" + JSON.stringify(slackMessage));
    }
};
